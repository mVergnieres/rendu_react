...
def update_movie_director(db: Session, movie_id: int, director_id: int):
    db_movies = get_movie(db=db, movie_id=movie_id)
    db_stars =  get_star(db=db, star_id=director_id)
    if db_movie is None or db_stars is None:
        return None
    # update object association
    db_movies.director = db_stars
...
def add_movie_actor(db: Session, movie_id: int, star_id: int):
    db_movies = get_movie(db=db, movie_id=movie_id)
    db_stars = get_star_by_id(db=db, star_id=star_id)
    if db_movies is None or db_stars is None:
        return None
    db_movies.actors.append(db_stars)
    db.commit()
    return db_movies
...
def update_movie_actors(db: Session, mid: int, sids: List[int]):
    Slist = db.query(models.Star).filter(models.Star.id.in_(sids)).all()
    db_movies = get_movie(db=db, movie_id=mid)
    if len(Slist) != len(sids):
        return None
    db_movies.actors = Slist
    db.commit()
    return db_movie

def update_star(db: Session, movie: schemas.Star):
    db_stars = db.query(models.Star).filter(models.Star.id == Star.id).first()
    if db_stars is not None:
        db_stars.name = star.name
        db_stars.birthdate = star.birthdate
        db.commit()
    return db_starss
...
def make_star(db: Session, star: schemas.StarCreate):
    db_stars = models.Star(name=star.name, birthdate=star.birthdate)
    db.add(db_stars)
    db.commit()
    db.refresh(db_stars)
    return db_stars
...
def dlt_movie(db: Session, movie_id: int):
     db_movie = db.query(models.Movie).filter(models.Movie.id == movie_id).first()
     if db_movie is not None:
         db.delete(db_movie)
         db.commit()
...
def dlt_star(db: Session, movie_id: int):
     db_stars = db.query(models.Star).filter(models.Star.id == star_id).first()
     if db_stars is not None:
         db.delete(db_stars)
         db.commit()
     return db_stars
...
def get_stats_by_actors(db: Session, min_count: int = 5):
    db_stats= db.query(models.Star,func.count(models.Movie.id).label("nombre_de_film"),func.min(models.Movie.year).label("premier_film"),func.max(models.Movie.year).label("dernier_film"))
    db_stats = db_stats.join(models.Movie.actors) \
        .group_by(models.Star) \
        .having(func.count(models.Movie.id) >= min_count) \
        .order_by(desc("nombre_de_film"))
    return [{"name": db_stat[0], "nombre_de_film": db_stat[1], "premier_film": db_stat[2], "dernier_film": db_stat[3]} for db_stat in db_stats]
